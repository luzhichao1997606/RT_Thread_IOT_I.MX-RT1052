...
{ 	 
	...
	FSL_Delay_us(6);//ADS7846的转换时间最长为6us
	TCLK(0); 	     	    
	FSL_Delay_us(1);    	   
	TCLK(1);		//给1个时钟，清除BUSY
	FSL_Delay_us(1);    
...
{		
    ...
    AT24CXX_ReadOneByte(SAVE_ADDR_BASE+13,&flag);//读取标记字,看是否校准过！	 
	if(flag==0X0A)//触摸屏已经校准过了			   
	{ 
		tempfac=AT24CXX_ReadLenByte(SAVE_ADDR_BASE,4);	   
		tp_dev.xfac=(float)tempfac/100000000.0f;//得到x校准参数
		tempfac=AT24CXX_ReadLenByte(SAVE_ADDR_BASE+4,4);	    
		tp_dev.yfac=(float)tempfac/100000000.0f;//得到y校准参数
	    //得到x偏移量
		tp_dev.xoff=AT24CXX_ReadLenByte(SAVE_ADDR_BASE+8,2);         
 	    //得到y偏移量
		tp_dev.yoff=AT24CXX_ReadLenByte(SAVE_ADDR_BASE+10,2);	         
 		//tp_dev.touchtype=AT24CXX_ReadOneByte(SAVE_ADDR_BASE+12);//读取触屏类型标记
        AT24CXX_ReadOneByte(SAVE_ADDR_BASE+12,&(tp_dev.touchtype));//读取触屏类型标记  
...
u8 TP_Init(void)
{	
    gpio_pin_config_t touch_config;
	...
	{
        //IO功能设置
        IOMUXC_SetPinMux(IOMUXC_GPIO_B1_12_GPIO2_IO28,0);	        //T_MOSI
        IOMUXC_SetPinMux(IOMUXC_GPIO_B1_13_GPIO2_IO29,0);           //T_SCK
        IOMUXC_SetPinMux(IOMUXC_SNVS_PMIC_STBY_REQ_GPIO5_IO02,0);   //T_CS
        IOMUXC_SetPinMux(IOMUXC_GPIO_AD_B0_08_GPIO1_IO08,0);        //T_MISO
        IOMUXC_SetPinMux(IOMUXC_GPIO_SD_B1_04_GPIO3_IO04,0);        //T_PEN
        
        IOMUXC_SetPinConfig(IOMUXC_GPIO_B1_12_GPIO2_IO28,0x10B0);
        IOMUXC_SetPinConfig(IOMUXC_GPIO_B1_13_GPIO2_IO29,0x10B0);
        IOMUXC_SetPinConfig(IOMUXC_SNVS_PMIC_STBY_REQ_GPIO5_IO02,0x10B0);
        IOMUXC_SetPinConfig(IOMUXC_GPIO_AD_B0_08_GPIO1_IO08,0xF080);    //输入
        IOMUXC_SetPinConfig(IOMUXC_GPIO_SD_B1_04_GPIO3_IO04,0xF080);    //输入
         
        touch_config.direction=kGPIO_DigitalOutput;	//输出
        touch_config.interruptMode=kGPIO_NoIntmode;	//不使用中断功能
        touch_config.outputLogic=1;					//默认高电平
        GPIO_PinInit(GPIO2,28,&touch_config); 	    
        GPIO_PinInit(GPIO2,29,&touch_config);     
        GPIO_PinInit(GPIO5,2,&touch_config); 
        
        touch_config.direction=kGPIO_DigitalInput;	//输入
        touch_config.interruptMode=kGPIO_NoIntmode;	//不使用中断功能
        touch_config.outputLogic=1;					//默认高电平
        GPIO_PinInit(GPIO1,8,&touch_config); 	    
        GPIO_PinInit(GPIO3,4,&touch_config); 
