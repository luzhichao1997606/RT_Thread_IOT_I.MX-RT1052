...
void LCD_Init(void)
{
    ...
    dbi_config.tCsxSetup_Ns = 101;                  //CS建立时间,控制CS拉低到WR/RD拉低的延迟,相当于控制地址建立时间
    dbi_config.tCsxHold_Ns = 34;                    //CS保持时间,在WR/RD拉高,并延迟RDH/WRH以后,CS再保持低电平的时间
    ...
    dbi_config.tCsxInterval_Ns = 34;                //写数据建立时间,控制CS高电平时间(连续操作时)
    SEMC_ConfigureDBI(SEMC, &dbi_config, semcclock); //初始化DBI接口
    FSL_Delay_ms(50);                                   // delay 50 ms
    //尝试9341 ID的读取
    LCD_WR_REG(0XD3);
    ...
    if (lcddev.id == 0X9341 || lcddev.id == 0X5310 || lcddev.id == 0X5510 || lcddev.id == 0X1963) //如果是这几个IC,则设置WR时序为最快
    {
        ...
        else SEMC->DBICR1 = 0 << 0;             //CS建立时间,控制CS拉低到WR/RD拉低的延迟,1CLK=6.67NS
        SEMC->DBICR1 = 0 << 0;                  //CS建立时间,控制CS拉低到WR/RD拉低的延迟,1CLK=6.7NS
